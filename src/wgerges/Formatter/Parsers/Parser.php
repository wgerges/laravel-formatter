...
abstract class Parser {
     ...
     * @return  string
     */
    private function xmlify($data, $structure = null, $basenode = 'xml', $parent=null) {
        ...
        foreach ($data as $key => $value) {
            ...
            if (is_numeric($key)) {
                // make string key...
                $flag = true;
                $key = (Str::singular($basenode) != $basenode) ? Str::singular($basenode) :  $structure->getName();                
            }
            ...
            if (is_array($value) or is_object($value)) {
                ...
                if(substr($key, 0, 1) == "_"){
                    $structure->addAttribute(str_replace('_', '', $key ), $value);
                } else {
